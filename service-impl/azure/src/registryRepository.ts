import {Pool} from "pg";
import {
  AppendValidatorRequest, Converter, CreateConverterRequest,
  CreateFormatRequest, DeleteConverterRequest,
  DeleteFormatRequest,
  Format, RemoveValidatorRequest,
  Schema,
  SetParentRequest, Validator
} from "../../../proto/lib/messages/riiid/kvf";
import {Type} from "../../../proto/lib/messages/riiid/kvf/Format";

export async function createFormat(pool: Pool, {formatName, formatDescription}: CreateFormatRequest): Promise<Format> {
...
export async function createValidator(
  pool: Pool,
  {formatId, validatorName, validatorDescription, validatorSrc, validatorIntegrity}: AppendValidatorRequest
) {
  const client = await pool.connect()
  const validatorQueryConfig = {
    text: `INSERT INTO validators(name, description, src, integrity) VALUES($1, $2, $3, $4) RETURNING *`,
    values: [validatorName, validatorDescription, validatorSrc, validatorIntegrity]
  }
  const validator: Validator = (await client.query(validatorQueryConfig)).rows[0]
  client.release()
  const format = await getFormat(pool, formatId)
  const validatorIds = (await getValidatorIds(format.validatorIds, validator.id)).map(res => res.toString())
...
export async function deleteValidator(
  ...
  const format = await getFormat(pool, formatId)
  const validatorIds = format.validatorIds.filter((formatValidatorId: string) => formatValidatorId != validatorId)
  await updateFormatValidatorIds(pool, formatId, validatorIds)
  client.release()
}
...
export async function createConverter(
  pool: Pool,
  {fromFormatId, toFormatId, converterName, converterDescription, converterSrc, converterIntegrity}: CreateConverterRequest
) {
  const client = await pool.connect()
  const converterSqlConfig = {
    text: `INSERT INTO converters(name, description, src, integrity) VALUES($1, $2, $3, $4) RETURNING *`,
    values: [converterName, converterDescription, converterSrc, converterIntegrity]
  }
...
export async function createEdge(pool: Pool, fromFormatId: string, toFormatId: string, converterId: string) {
  const client = await pool.connect()
  const sqlConfig = {
...
export async function getValidatorIds(validatorIds: string[], validatorId: string) {
  if (validatorIds) {
