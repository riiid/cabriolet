...
describe("CreateRegistryTest", () => {
  ...
  describe("deleteFormat", () => {
    it("when delete format then format does not exist in formats", async () => {
      ...
      expect(beforeSchema.schema!!.formats[0].description).toEqual(formatDescription)
    ...
    it("when delete parent then parent format id does not exist in the format", () => {
      ...
      registry.createFormat({
        ...
        .then(res => {
          ...
          // then
          beforeSchema.then(schema => expect(schema.schema!!.formats[0].parentFormatId).toEqual(parentFormatId))
          beforeSchema.then(schema => expect(schema.schema!!.formats[0].id).toEqual(formatId))
          afterSchema.then(schema => expect(schema.schema!!.formats[0].parentFormatId).toEqual("undefined"))
        })
    })
  })
  ...
  describe("appendValidator", () => {
    it("when create validator then validatorId is added in format successfully and validator is added successfully", async () => {
      ...
      const validatorDescription = crypto.randomUUID() + "_validatorDescription"
        ...
        .then(res => {
          ...
          registry.getSchema({})
            .then(res => {
              ...
              expect(schema!!.formats[0].validatorIds[0]).toBe(validatorId.toString())
  ...
  describe("createConverter", () => {
    it("when create converter then edge and converter are added", async () => {
      ...
      const formatName = crypto.randomUUID() + "_formatName"
      const formatDescription = crypto.randomUUID() + "_formatDescription"
      const fromFormatId = (await registry.createFormat({
        formatName: formatName,
        formatDescription: formatDescription
      })).formatId
      const toFormatId = (await registry.createFormat({
        formatName: formatName,
        formatDescription: formatDescription
      })).formatId
      const converterName = crypto.randomUUID() + "_converterName"
      const converterDescription = crypto.randomUUID() + "_converterDescription"
      ...
      // then
      expect(schema.schema!!.edges[0].toFormatId).toEqual(toFormatId.toString())
      expect(schema.schema!!.edges[0].fromFormatId).toEqual(fromFormatId.toString())
      expect(schema.schema!!.edges[0].converterId).toEqual(converterId.converterId.toString())
      ...
      expect(schema.schema!!.converters[0].integrity).toEqual(converterIntegrity)
    })
  })
