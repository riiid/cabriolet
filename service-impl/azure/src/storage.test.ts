...
beforeAll( () => {
  try {
    ...
    const azureStorageConfig: AzureStorageConfig = {azureContainerClient: containerClient}
...
describe("AzureStorageTest", () => {
  describe("get", () => {
    test("when get blob after saving one, then retrieves successfully", async () => {
      ...
      await storage.set({key: key, value: new Uint8Array(rndInt), formatId: formatId})
