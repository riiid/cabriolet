...
export default function createAzureStorage(
  ...
  return {
    async has({ key }) {
      return { has: await config.azureContainerClient.getBlobClient(key).exists() }
    },
    async get({ key }) {
      const blobClient = await config.azureContainerClient.getBlockBlobClient(key)
      ...
      return { value: new Uint8Array(blob.buffer), formatId: blobMetadata.contentType!! }
    },
    async getFormatId({ key }) {
      const blobClient = await config.azureContainerClient.getBlobClient(key).download(0)
    ...
    async set({ key, value, formatId }) {
      ...
      await config.azureContainerClient.uploadBlockBlob(key, value, value.length, options)
      return {}
    },
    async delete({ key }) {
    ...
    async keys({ prefix }) {
      const iter = await config.azureContainerClient.listBlobsFlat({prefix: prefix})
      const res: string[] = []
